...
export function verifyStageB(root = MODULE_ROOT, injectedPolicy = null, injectedCapsules = null, injectedEnv = process.env, ciMode = injectedEnv.__ciMode === true) {
  try {
...
if (process.argv[1] && path.resolve(process.argv[1]) === path.resolve(fileURLToPath(import.meta.url))) { const ciMode = process.argv.includes('--ci'); const result = verifyStageB(MODULE_ROOT, null, null, process.env, ciMode); process.stdout.write(`${JSON.stringify(result)}\n`); process.exitCode = result.decision === 'stage-b-admitted-unverified' ? 0 : 2; }
